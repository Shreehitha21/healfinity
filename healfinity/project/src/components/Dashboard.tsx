...
const Dashboard: React.FC<DashboardProps> = ({ healthData, consultations }) => {
  ...
  const { getHealthData } = useDatabase();
  ...
  // Use current health data or fallback to defaults
  const displayHealthData = {
    steps: currentHealthData?.steps || 0,
    heartRate: currentHealthData?.heartRate || 0,
    sleep: currentHealthData?.sleep || 0,
    water: currentHealthData?.water || 0
  };
              ...
              <div key={index} className="bg-white/50 rounded-lg p-4 border border-white/20">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="font-medium text-gray-900">{consultation.doctor_name || consultation.doctor}</h3>
                ...
                <button 
                  onClick={() => alert(`🎥 Joining ${consultation.type} consultation with ${consultation.doctor_name || consultation.doctor}...\n\nPlease wait while we connect you to the doctor.`)}
